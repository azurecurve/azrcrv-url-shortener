 ...
 *
 */
function azrcrv_urls_set_default_options($networkwide){
	
	$option_name = 'azrcrv-urls';
	$old_option_name = 'azc_urls';
	...
	if (function_exists('is_multisite') && is_multisite()){
		...
		if ($networkwide){
			...
			foreach ($blog_ids as $blog_id){
				...
				azrcrv_urls_update_options($option_name, $new_options, false, $old_option_name);
			}
		...
		if (get_site_option($option_name) === false){
			azrcrv_urls_update_options($option_name, $new_options, true, $old_option_name);
		}
	}
 ...
 *
 */
function azrcrv_urls_update_options($option_name, $new_options, $is_network_site, $old_option_name){
 ...
 *
 */
function azrcrv_urls_update_default_options( &$default_options, $current_options ) {
...
function azrcrv_urls_add_plugin_action_link($links, $file){
	...
	if ($file == $this_plugin){
		$settings_link = '<a href="'.get_bloginfo('wpurl').'/wp-admin/admin.php?page=azrcrv-urls"><img src="'.plugins_url('/pluginmenu/images/Favicon-16x16.png', __FILE__).'" style="padding-top: 2px; margin-right: -5px; height: 16px; width: 16px;" alt="azurecurve" />'.esc_html__('Settings' ,'url-shortener').'</a>';
